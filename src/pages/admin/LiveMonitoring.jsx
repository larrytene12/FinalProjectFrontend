...
import { AlertTriangle, Ban, Signal, Eye, CheckCircle, RefreshCw } from 'lucide-react';
...
const LiveMonitoring = () => {
  ...
  // Simulasi Real-time Data Fetching (Auto Refresh tiap 2 detik)
  useEffect(() => {
    fetchData();
  ...
  const handleKick = async (id, name) => {
    if(window.confirm(`FORCE LOGOUT peserta ${name}? Tindakan ini tidak bisa dibatalkan.`)) {
        try {
            ...
            fetchData(); // Langsung refresh data agar UI berubah
        } catch (error) {
            alert("Gagal melakukan aksi kick. Cek koneksi server.");
        }
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
        ...
        {/* CCTV GRID */}
        {loading ? (
            <div className="text-center p-10 text-slate-400">Menghubungkan ke kamera pengawas...</div>
        ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {students.filter(s => s.examStatus === 'Sedang Ujian' || s.examStatus === 'Diskualifikasi').length === 0 && (
                    <div className="col-span-3 text-center p-12 bg-slate-100 rounded-2xl border-2 border-dashed border-slate-300 text-slate-500">
                        <Signal size={48} className="mx-auto mb-4 text-slate-300"/>
                        <p>Tidak ada peserta yang sedang ujian saat ini.</p>
                    </div>
                )}

                {students.filter(s => s.examStatus === 'Sedang Ujian' || s.examStatus === 'Diskualifikasi').map((student, idx) => {
